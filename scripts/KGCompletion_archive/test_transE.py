"""
Case study and analysis of TransE
Author: Jiaying Lu
...
from model.TransE import TransE, cal_metrics, get_ranked_predction
from .test_taxotransE import cal_taxo_nontaxo_metrics


# Sacred Setup
ex = sacred.Experiment('test-TransE')
...
def test(model: th.nn.Module, data_loader: DataLoader, ent_count: int, device: th.device,
         known_triples_map: dict) -> Tuple[float, float, float, float, tuple]:
    hits_1 = 0.0
    hits_3 = 0.0
    hits_10 = 0.0
    mrr = 0.0
...
def case_study(model: th.nn.Module, data_loader: DataLoader, ent_count: int, device: th.device,
               known_triples_map: dict, ent_vocab: dict, rel_vocab: dict):
...
def test_model(config_path, checkpoint_path, _run, _log):
    ...
    # Build model
    model = TransE(len(ent_vocab), len(rel_vocab), opt['dist_norm'], opt['emb_dim'])
    ...
    # test
    model.eval()
    hits_1, hits_3, hits_10, mrr, metrics_rels = test(model, test_iter, len(ent_vocab), device, all_triples_map)
    ...
    case_study(model, test_iter, len(ent_vocab), device, all_triples_map, ent_vocab, rel_vocab)
