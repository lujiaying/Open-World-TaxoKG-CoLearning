...
class AttTaxoTransE(nn.Module):
    def __init__(self, ent_count: int, rel_count: int, norm: int = 1,
        ...
        self.attn_scorer_p = nn.Sequential(
                nn.Linear(dim*3, attn_dim, bias=False),
    ...
    def _cal_attn_score(self, emb_s: th.FloatTensor, emb_r: th.FloatTensor, emb_n: th.FloatTensor,
        ...
        h_r = emb_r.unsqueeze(1).repeat(1, max_len, 1)  # (batch, max_l, dim)
        attn_scores = attn_scorer(th.cat((h_s, emb_n, h_r), dim=-1))   # (batch, max_l, 1)
    ...
    def _aggregate_over_taxo(self, ents: th.LongTensor, emb_r: th.FloatTensor, taxo_dict: dict) -> th.FloatTensor:
        ...
        attn_score_p = self._cal_attn_score(emb_s, emb_r, emb_p, mask_p, True)   # (batch, max_l)
        attn_score_c = self._cal_attn_score(emb_s, emb_r, emb_c, mask_c, False)  # (batch, max_l)
    ...
    def forward(self, triples: th.LongTensor, taxo_dict: dict) -> th.FloatTensor:
        ...
        h = self._aggregate_over_taxo(triples[:, 0], r, taxo_dict)
        t = self._aggregate_over_taxo(triples[:, 2], r, taxo_dict)
        return self._cal_distance(h, r, t)

    def predict(self, triples: th.LongTensor, all_embs_rels: th.Tensor) -> th.FloatTensor:
        """
        Args:
            triples: shape=(batch*ent_c,3)
            all_embs_rels: shape=(rel_c,ent_c,dim)
        """
        r = self.rel_emb(triples[:, 1])
        h = all_embs_rels[triples[:, 1], triples[:, 0], :]
        t = all_embs_rels[triples[:, 1], triples[:, 2], :]
        return self._cal_distance(h, r, t)
